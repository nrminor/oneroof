...
`--ref_gbk`               | None          | The reference sequence to be used for variant annotation in Genbank format. Can also use .gff or .gff3 files.
`--primer_tsv`            | None          | A TSV file of user provided amplicon names, forward primers and reverse primers. To be used instead of `--primer_bed` if the user has specific primers they want to use. The TSV file needs to have the headers: `amplicon_name`, `fwd_sequence` and `reverse_sequence`
...
`--meta_ref`              | None          |  Dataset, either a local FASTA file or a pre-built dataset built by Sylph, to use for metagenomic profiling. Can download prebuilt datasets here: https://github.com/bluenote-1577/sylph/wiki/Pre%E2%80%90built-databases.
