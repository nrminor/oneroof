...
- Package entry point
- Imports and runs the `oneroof` CLI
- Allows running as `python -m bin`

### OneRoof CLI

The CLI package has moved to `lib/oneroof_cli/`. See the lib/ Directory section above for details.
...
- Controls which reports are generated

**multiqc_config.yaml**

- MultiQC report configuration template
- Controls section ordering and appearance
- Defines search patterns for custom content
- Dynamic values (platform, reference) are substituted at runtime
...
Library code for Nextflow (Groovy) and Python packages.
...
The `lib/` directory contains two Python packages that are installed as part of the project:

**oneroof_cli/** - Command-line interface package

- `__init__.py` - Package initialization and main entry point
- `app.py` - Typer application configuration
- `utils.py` - Shared utilities (console output, Nextflow command generation)
- `commands/` - CLI command implementations (run, resume, env, validate)

**reporting/** - Reporting and visualization package

- `__init__.py` - Package exports
- `schema.py` - Pydantic models for JSON report schema
- `multiqc.py` - MultiQC custom content TSV generators
- `extractors/` - Metric extraction modules:
  - `alignment.py` - BAM file metrics
  - `consensus.py` - Consensus FASTA metrics
  - `coverage.py` - Per-base coverage metrics
  - `haplotyping.py` - Devider output metrics (Nanopore only)
...
- `visualizations/` - Altair chart generators:
  ...
  - `amplicon_efficiency.py` - Amplicon performance charts
  - `coverage_heatmap.py` - Coverage visualization
  - `qc_dashboard.py` - QC status charts
  - `variant_summary.py` - Variant type/effect charts

### Library Tests

\*\*test\_\*.py files\*\* in `lib/`

- Unit tests for the reporting package
- Tests for extractors, visualizations, and MultiQC generators
- Run with `uv run pytest lib/`
...
- Python script for Quarto post-render processing
